...
function initAboutPage() {
  ...
  if (!hasVisited) {
    ...
    if (nav) nav.classList.add("first-load");
...
function updateCarousel() {
  ...
  items.forEach((item, index) => {
    ...
    if (index === currentVideoIndex) {
      ...
      if (video) video.currentTime = 0; // Optional: can remove
...
// No more seekVideo based on thumbnails needed,
// but keeping seek function for slider:
